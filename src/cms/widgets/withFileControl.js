...
const Image = _styled.img`
  width: 100%;
  height: 100%;
  object-fit: contain;
`;

const MultiImageWrapper = _styled.div`
  display: flex;
  flex-wrap: wrap;
`;

const FileLink = _styled.a`
  margin-bottom: 20px;
  font-weight: normal;
  color: inherit;
  &:hover,
...
const FileLinks = _styled.div`
  margin-bottom: 12px;
`;

const FileLinkList = _styled.ul`
  list-style-type: none;
`;

const FileWidgetButton = _styled.button`
  ${buttons.button};
  ${components.badge};
`;

const FileWidgetButtonRemove = _styled.button`
  ${buttons.button};
...
export default function withFileControl({ forImage } = {}) {
  return class FileControl extends React.Component {
    ...
    renderSelection = subject => {
      ...
      return <div>
        <MediaThumbnail src={this.props.value} height={100} />
        <div>
